...
static func _lacaio() -> EnemyData:
	var e := EnemyData.make("lacaio", "Lacaio", 24, 140.0, 5, 2, 3, 14.0, Color(0.85, 0.3, 0.3))
...
static func _espectro() -> EnemyData:
	var e := EnemyData.make("espectro", "Espectro Veloz", 16, 245.0, 6, 2, 2, 12.0, Color(0.55, 0.7, 0.95))
	e.attack_rate = 1.4
...
static func _esqueleto() -> EnemyData:
	var e := EnemyData.make("esqueleto", "Soldado Esqueleto", 70, 80.0, 15, 3, 4, 16.0, Color(0.8, 0.8, 0.72))
	e.defense = 4
...
static func _hidra() -> EnemyData:
	var e := EnemyData.make("hidra", "Hidra Menor", 46, 118.0, 12, 2, 3, 17.0, Color(0.3, 0.7, 0.4))
...
static func _hidra_filhote() -> EnemyData:
	var e := EnemyData.make("hidra_filhote", "Filhote de Hidra", 16, 150.0, 4, 1, 2, 10.0, Color(0.45, 0.85, 0.55))
	e.element = Elements.E.AGUA
	return e
...
static func _centauro() -> EnemyData:
	var e := EnemyData.make("centauro", "Centauro", 105, 178.0, 22, 4, 6, 18.0, Color(0.6, 0.4, 0.25))
	e.attack_rate = 1.2
	e.defense = 3
	e.element = Elements.E.TERRA
	return e
...
static func _ciclope() -> EnemyData:
	var e := EnemyData.make("ciclope", "Ciclope", 220, 70.0, 40, 5, 11, 22.0, Color(0.45, 0.45, 0.5))
	e.defense = 7
...
static func _talos() -> EnemyData:
	var e := EnemyData.make("talos", "Talos, o Colosso", 700, 60.0, 160, 8, 16, 30.0, Color(0.75, 0.55, 0.25))
	e.defense = 12
