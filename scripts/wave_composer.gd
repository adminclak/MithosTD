...
static func compose(stage_index: int, wave_index: int, total_waves: int) -> Array:
	var w := wave_index
	match stage_index:
		1: # Campos de Elis — tutorial: só lacaios (mas em volume que exige >1 torre).
			return [{"id": "lacaio", "count": 7 + 2 * w}]
		2: # Bosque de Nemeia — espectros velozes (ensina o bloqueio).
			return [{"id": "lacaio", "count": 5 + w}, {"id": "espectro", "count": 2 + w}]
		3: # Pantano da Hidra — hidras que se dividem (premia AoE).
			return [{"id": "lacaio", "count": 4 + w}, {"id": "hidra", "count": 2 + int(w / 2.0)}]
		4: # Desfiladeiro dos Centauros — mistura pesada.
			var g4 := [{"id": "lacaio", "count": 4 + w}, {"id": "esqueleto", "count": 2 + int(w / 2.0)}]
			if w >= 3:
				g4.append({"id": "centauro", "count": 1 + int((w - 2) / 2.0)})
			return g4
		5: # Encosta do Olimpo — boss Talos na onda final.
			if w >= total_waves:
				return [{"id": "lacaio", "count": 10}, {"id": "ciclope", "count": 3}, {"id": "talos", "count": 1}]
			var g5 := [{"id": "lacaio", "count": 5 + w}, {"id": "esqueleto", "count": 2 + int(w / 2.0)}]
			if w >= 4:
				g5.append({"id": "ciclope", "count": 1 + int((w - 3) / 2.0)})
			return g5
	return [{"id": "lacaio", "count": 6 + 2 * w}]
